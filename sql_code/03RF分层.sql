...
-- 口径：
--   * R（近度）：最近行为距分析末日 2017-12-03 的天数。≤1天=5档 ≤3天=4档 ≤7天=3档 ≤15天=2档 >15天=1档
--     ⚠️ 数据窗口仅 9 天，R 最大到 9 → 15 天档（档1）窗口内无用户，实际观测 5/4/3/2 四档
--   * F（频次）：9 天内购买次数。≥5次=5档 3-4次=4档 2次=3档 1次=2档 0次=1档
--   * 分层：高价值=R≥4且F≥4；潜力=R≥3且F≥3；其余=普通
...
WITH user_rf AS (
    SELECT user_id,
        ...
        COUNT(CASE WHEN behavior_type = 'buy' THEN 1 END) AS F_count
    FROM ub_clean
    GROUP BY user_id
